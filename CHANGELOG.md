...
- **应用内更新（Windows）**：安装新版本时报错"请求的操作需要提升 (740)"——NSIS 安装器需要管理员权限，现在通过系统 UAC 弹窗触发提升，能正常拉起安装器。

### 新增

- **启动自动检查更新**：进入主窗口后会在后台静默检查一次。有新版本时弹出更新对话框；没有新版本完全无感（不弹任何提示）。
- **下载缓存**：在更新对话框下载完安装包后，如果关闭对话框再次进入更新流程，会直接跳过下载、提示安装，省去重复等待。
