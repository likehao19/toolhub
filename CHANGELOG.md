...
- **接口调试 / 工具箱页面**：顶部"环境选择"、GET/POST 方法选择器以及工具箱内所有下拉选择框（el-select）显示宽度修复——之前 select 容器被撑得很宽但里面真实可见区域只有一小段，现在内部撑满外层容器，视觉对齐。
- **接口调试 — Params/Headers/Body 标签栏**：悬浮时不再有蓝色底色；active 状态的下划线改为等长居中，所有标签下方的下划线一样长。

### 变更

- **接口调试 / 接口文档**：移除左侧侧边栏的折叠按钮，侧栏改为常驻显示。
- **官网下载页**：所有下载按钮直接指向当前版本的具体安装包文件，点击即下，不再绕道 GitHub Releases 列表页。
