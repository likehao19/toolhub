...
export async function initReminderService() {
  ...
  checkInterval = setInterval(async () => {
    try {
      ...
      console.warn('[ReminderService] 定时检查失败:', error)
    }
  }, 30000) // 30秒检查一次
  ...
  try {
    ...
    console.warn('[ReminderService] 初始检查失败:', error)
  }
}
...
async function ensureReminderColumns() {
  ...
  try {
    ...
    console.warn('[ReminderService] 检查/添加字段失败:', error)
  }
}
  ...
  try {
    ...
    for (const todo of todos) {
      ...
      if (reminderType === 'on_start' && todo.start_date === today) {
        ...
        const reminderDateStr = formatLocalDate(reminderDate)
    ...
    console.warn('[ReminderService] 检查待办提醒失败:', error)
  }
}
...
async function checkEventReminders() {
  ...
  try {
    ...
    console.warn('[ReminderService] 检查日程提醒失败:', error)
  }
}
...
async function getReminderConfig() {
  try {
    ...
    console.warn('[ReminderService] 读取提醒配置失败:', error)
  }
